...
WinMaliDxgkPatchInitializationData(_Out_ DRIVER_INITIALIZATION_DATA* init)
{
    ...
    /* We compile against the 1809 WDK -> DXGKDDI_INTERFACE_VERSION is
       WDDM 2.5. dxgkrnl validates every field the struct exposes at the
       declared version: any NULL callback there is rejected with
       STATUS_REVISION_MISMATCH. So the implementation strategy is
       "stub everything, hand-implement what we need".

       The wire-up block below is generated by Tools\gen-dxgk-stubs.py
       from the struct definition in dispmprt.h. Re-run that script
       after any WDK upgrade. */

    /* Mali G610 has a real GPU-side MMU, full LPAE page tables, async
       CSF submission - architecturally it's a WDDM 2.5 adapter. Pin
       init.Version to WDDM 2.5 to match. DRIVERCAPS.WDDMVersion is
       updated to match in WinMaliQai.c (both must align or dxgkrnl
       returns STATUS_REVISION_MISMATCH at DriverInitialize). */
