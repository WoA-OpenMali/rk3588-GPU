...
WinMaliHwProbeIdentity(_Inout_ PWINMALI_HW Hw)
{
    ...
    if (Hw->RegsVa == NULL) {
        // No MMIO mapped yet - leave cached IDs at zero. Not an error for
        // the skeletal build, but future callers that assume Hw->GpuId != 0
        // should check WinMaliHwIsMaliG610() first.
        WINMALI_TRACE("no regs mapped, deferring identity probe");
        return STATUS_SUCCESS;
    }
    ...
    if (Hw->GpuId == 0xFFFFFFFFu || Hw->GpuId == 0x00000000u) {
        WINMALI_ERROR("GPU_ID reads as 0x%08x - is the GPU power domain on?",
                      Hw->GpuId);
        // Don't decode the field - ProdMajor==0xF on 0xFF.. is meaningless.
...
WinMaliHwProductName(_In_ const WINMALI_HW* Hw)
{
    ...
    // Matches panthor_hw.c GPU_PROD_ID_MAKE() table. Only the parts that
    // appear in the Valhall lineage are listed here; anything else falls
    // through to "unknown". We intentionally don't distinguish
    // Immortalis vs Mali variants by shader-core count yet - that needs
    // GPU_CORE_FEATURES + GPU_SHADER_PRESENT when feature query is extended.
